<?php
/**
 * Marks a tag as deleted in the database. The tag is not actually deleted completely, allowing existing events and report data to continue working.
 *
...
if (isSustainabilityTeamUser($session_permission)) {
    switch ($method) {
        case 'POST':
            ...
            $stmt = $connection->prepare("UPDATE FoodTag SET isDeleted = ? WHERE Tag_ID = ?");
            $stmt->bind_param("is",$newDeletedState,$id);
            if ($stmt->execute()){
                $response->wasSuccessful = 1;
                $response->message = "The tag was removed from the application.";
            } else {
                $response->wasSuccessful = 0;
                $response->message = "There was an issue removing the tag";
            }
            break;
        default:
